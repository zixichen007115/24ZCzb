...
def j_init_move(act=None):
    ...
    pos_list = np.zeros((3, ctrl_step + 1))
    ctrl_num = 0
...
for i in range(2):
    act_add = np.zeros(2)
    act_add[i] = 0.1
    k_cols_add = j_init_move(act_add)

    act_min = np.zeros(2)
    act_min[i] = -0.1
    k_cols_min = j_init_move(act_min)

    j_col = (k_cols_add[:, -1] - k_cols_min[:, -1]) / 0.2

    k_init[:, i] = j_col[:2]

print(k_init)
np.save('../0_files/k_init.npy', k_init)

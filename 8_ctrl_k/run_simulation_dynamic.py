...
def main(ctrl_step=1, tar_list=None):
    ...
    poses = np.zeros([t_step, 2])
    acts = np.zeros([t_step - 1, 2])

    act_list = np.zeros((ctrl_step, 2))
    pre_act = np.zeros(2)

    k = np.load("../0_files/k_init.npy")
    ...
    for k_sim in tqdm(range(total_steps)):
        if (k_sim % controller_step_skip) == 0:
            ...
            poses[:-1] = poses[1:]
            poses[-2] = real_list[:, ctrl_num]
            poses[-1] = tar_list[:, ctrl_num]

            acts[:-1] = acts[1:]
            acts[-1] = np.copy(pre_act)

            act, k = act_est(poses, acts, k)

            act_noise = np.random.rand(2) * 0.1 - 0.05
            act_real = act + act_noise

            act_max = 1
            for i in range(2):
                if np.abs(act_real[i]) > act_max:
                    act_real[i] = act_max * np.sign(act_real[i])

            pre_act = np.copy(act)
            act_list[ctrl_num] = act_real

            activations[0] = np.ones(100) * np.max([0, act_real[0]])
            activations[1] = np.ones(100) * np.max([0, act_real[1]])
            activations[2] = np.ones(100) * np.max([0, -act_real[0]])
            activations[3] = np.ones(100) * np.max([0, -act_real[1]])
